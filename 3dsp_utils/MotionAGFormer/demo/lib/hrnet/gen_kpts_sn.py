...
from MotionAGFormer.demo.lib.hrnet.lib.utils.utilitys import plot_keypoint, PreProcess, write, load_json
from MotionAGFormer.demo.lib.hrnet.lib.config import cfg, update_config
from MotionAGFormer.demo.lib.hrnet.lib.utils.transforms import *
from MotionAGFormer.demo.lib.hrnet.lib.utils.inference import get_final_preds
from MotionAGFormer.demo.lib.hrnet.lib.models import pose_hrnet

cfg_dir = 'MotionAGFormer/demo/lib/hrnet/experiments/'
model_dir = 'MotionAGFormer/demo/lib/checkpoint/'

# Loading human detector model
from MotionAGFormer.demo.lib.yolov3.human_detector import load_model as yolo_model
from MotionAGFormer.demo.lib.yolov3.human_detector import yolo_human_det as yolo_det
from MotionAGFormer.demo.lib.sort.sort import Sort
...
def gen_video_kpts_sn(args2,video, det_dim=416, num_peroson=1, gen_output=False,shooter_tracklet_path=None):
    # Updating configuration
    args = parse_args()
    reset_config(args)
    ...
    video_length = args2.num_frame

    kpts_result = []
    scores_result = []
    #load shooter tracklet txt file
    shooter_tracklet = pd.read_csv(shooter_tracklet_path,sep=',',header=None)
    #load the info csv
    
    #set column names
    shooter_tracklet.columns = ['frame','id','t','l','w','h','score','a','b','c']


    frame_count=1
    for ii in tqdm(range(video_length)):
        # ret, frame = cap.read()
        frame = cv2.imread(video + '/' + str(ii+1) + '.jpg')

        # if not ret:
        #     continue
        
        #get shooter bbox
        shooter_bbox = shooter_tracklet[shooter_tracklet['frame']==frame_count]
        if shooter_bbox.empty:
            print ("error: no bbox found for frame ",frame_count)
            continue
        
        x1=50-shooter_bbox['w'].values[0]/2
        y1=50-shooter_bbox['h'].values[0]/2
        x2=50+shooter_bbox['w'].values[0]/2
        y2=50+shooter_bbox['h'].values[0]/2
        
        bboxs = np.array([[x1,y1,x2,y2]])
        track_bboxs=bboxs #track_bboxs = [[x1,y1,x2,y2]]]
        frame_count += 1
        with torch.no_grad():
            # bbox is coordinate location
            # pdb.set_trace()
