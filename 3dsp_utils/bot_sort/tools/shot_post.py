...
from bot_sort.yolox.data.data_augment import preproc
from bot_sort.yolox.exp import get_exp
from bot_sort.yolox.utils import fuse_model, get_model_info, postprocess
from bot_sort.yolox.utils.visualize import plot_tracking
from bot_sort.tracker.bot_sort_app import BoTSORT
from bot_sort.tracker.tracking_utils.timer import Timer
...
def make_parser():
    ...
    parser.add_argument("--save_result", action="store_false",help="whether to save the inference result of image/video")
    ...
    parser.add_argument("--match_thresh", type=float, default=0.8, help="matching threshold for tracking") #hyperparameter
    parser.add_argument("--aspect_ratio_thresh", type=float, default=1.6, help="threshold for filtering out boxes of which aspect ratio are above the given value.")
    parser.add_argument('--min_box_area', type=float, default=10, help='filter out tiny boxes')
    parser.add_argument("--fuse-score", dest="fuse_score", default=False, action="store_true", help="fuse score and iou for association") #hyperparameter

    # CMC
    parser.add_argument("--cmc-method", default="orb", type=str, help="cmc method: orb | sift | ecc | sparseOptFlow ")
...
def sn_demo(file_path,detector_param_path,vis_folder,file, args,args2):
...
if __name__ == "__main__":
    ...
    #path
    detector_param_path="/home/c_yeung/workspace6/python/shot_posture/BoT-SORT/yolov8_player/best.pt"
    out_path ="/home/c_yeung/workspace6/python/shot_posture/BoT-SORT/results"
    os.makedirs(out_path, exist_ok=True)

    # main(exp, args)
    current_time = time.localtime()
    file_path="/data_pool_1/shot_posture/epl_clip_0.5s/all_clip"
    file_list=os.listdir(file_path)
    for file in file_list:
        sn_demo(os.path.join(file_path,file),detector_param_path,out_path,file, args)
